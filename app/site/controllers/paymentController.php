...
//		echo '</pre>';
		...
		if ($ac === 'add') {
			...
			if ($product_id) {
				if ($friend) {
					$this->cart->fr_products[$product_id] = array('product_id' => $product_id, 'qty' => 1, 'option_id' => $option_id, 'friend' => $friend);
				} else {
		...
		if ($ac === 'qty') {
			...
			if ($product_id) {
				$friend = $this->getParam('friend');
				if ($friend == 1) {
					$this->cart->fr_products[$product_id]['qty'] = $i;
				} else {
		...
		if ($ac === 'remove') {
			...
			if ($product_id) {
				$friend = $this->getParam('friend');
				if ($friend == 1) {
					unset($this->cart->fr_products[$product_id]);
				} else {
		...
		foreach ($this->cart->products AS $shopCartProduct) {
			...
			$op = $form->createElement('select', 'cart_op_0_'.$product['id']);
			$op->setRequired();
			...
			$qty = $form->createElement('select', 'cart_qty_0_'.$product['id']);
			$qty->setRequired();
...
//			for ($x = 1; $x <= $optionObj->stock - $optionObj->sold; $x++) {
		...
		foreach ($this->cart->fr_products AS $shopCartProduct) {
			if (!$shopCartProduct['product_id'] || !$shopCartProduct['option_id']) {
				unset($this->cart->fr_products);
				return array();
				//unset($this->cart->products[])
			}
			...
			$optionObj = ProductPriceTable::getInstance()->find($shopCartProduct['option_id']);
			$products_f[$product['id']] = $productObj;
			$options_f[$product['id']] = $optionObj;
			//print_r($options);
			if ($shopCartProduct['product_id'] <> $optionObj->product_id) {
				//return array();
			}
			...
			if ($optionObj->sold >= $optionObj->stock) {
				unset($this->cart->fr_products[$product['id']]);
				unset($products_f[$product['id']]);
				//return array('err' => 'Offer sold out');
			}
			...
			$ref = $form->createElement('text', 'ref_'.$product['id']);
			$ref->setRequired(false)
							->setValue($shopCartProduct['fid'])
							->addFilter('StringToLower');
			
			$op = $form->createElement('select', 'cart_op_1_'.$product['id']);
			$op->setRequired();
			...
			$qty = $form->createElement('select', 'cart_qty_1_'.$product['id']);
			$qty->setRequired();
...
//			for ($x = 1; $x <= $optionObj->stock - $optionObj->sold; $x++) {
			...
			$form->addElement($ref);
			$form->addElement($op);
			$formMDA1[$product['id']]['options'] = $op->getMultiOptions();
			$formMDA1[$product['id']]['qty'] = $qty->getMultiOptions();
		} //foreach
...
//							->addElement($msg);
		...
		if ($this->request->getPost()) {
			if ($form->isValid($this->request->getPost())) {
				$err = 0;
				$productsFORK = array();
				$spendFrom =  $_POST['kkt'];//1 wallet 2 wmw
...
//				if (!$this->cart->products) {
				if (!$this->cart->products && !$this->cart->fr_products) {
					$err = 1;
...
//				}
				...
				foreach ($this->cart->products AS $shopCartProduct) {
					...
					$quantity = $form->getValue('cart_qty_0_'.$product['id']);
					//$showDepositButton = false;
					if ($spendFrom == 1) {
						if ($shopCartProduct['product_id'] && ($this->user->getUser()->get('wallet') < $optionObj->get('price') * $quantity)) {
							$err = 1;
					...
					if ($spendFrom == 2) {
						if ($shopCartProduct['product_id'] && ($this->user->getUser()->get('virtual_wallet') < $optionObj->get('price') * $quantity)) {
							$err = 1;
							$form->addError('You do not have enought money in your wmw wallet to buy this product, please choose another payment option in order to buy this product.');
							//$showDepositButton = true;
						}
					}
					if (isset($kkt[$optionObj->get('id')])) {
						$kkt[$optionObj->get('id')]['total'] = $kkt[$optionObj->get('id')]['total']-$quantity;
					} else {
						$kkt[$optionObj->get('id')]['total'] = $optionObj->get('stock') - $optionObj->get('sold')-$quantity;
						$kkt[$optionObj->get('id')]['name'] = $product['name'];
					}
					if ($kkt[$optionObj->get('id')]['total'] < 0) {
						$err = 1;
						$form->addError($product['name'].' product is out of stock.');
					}
					//check product stock
...
//					}
				...
				$redir = 0;
				foreach ($this->cart->fr_products AS $key=>$shopCartProduct) {
					$productObj = ProductTable::getInstance()->getProductById(array('product_id' => $shopCartProduct['product_id']))->fetchOne(array());
					$product = $productObj->toArray();
					
					$fr = UserTable::getInstance()->findOneBy('ref_id', $form->getValue('ref_'.$shopCartProduct['product_id']));
					if (!$fr->id) {
						$err = 1;
						$form->addError('No user whit this "'.$form->getValue('ref_'.$shopCartProduct['product_id']).'" referral id.');
					}
					if ($_POST['submit'] == 'Save And Continue shopping!') {
						$this->cart->fr_products[$key]['fid'] = $form->getValue('ref_'.$shopCartProduct['product_id']);
						$redir = 1;
					}
					$productsFORK[] = array_merge($shopCartProduct, array('friend_id' => $fr->id));
					$optionObj = ProductPriceTable::getInstance()->find($shopCartProduct['option_id']);
					//check if the product exist
					if (!$product['id']) {
						$err = 1;
...
//					}
					//check if the user has enough money in wallet
					$quantity = $form->getValue('cart_qty_1_'.$product['id']);
					//$showDepositButton = false;
					if ($spendFrom == 1) {
						if ($shopCartProduct['product_id'] && ($this->user->getUser()->get('wallet') < $optionObj->get('price') * $quantity)) {
							$err = 1;
					...
					if ($kkt[$optionObj->get('id')]['total'] < 0) {
						$err = 1;
						$form->addError('Product "'.$product['name'].'" is out of stock.');
					}
					//check product stock
					...
					$totalCHK = $totalCHK + $optionObj->get('price') * $quantity;
					if ($this->cart->fr_products[$product['id']]['qty'] > $product['max_buy']) {
						$err = 1;
						$form->addError('The quantity is higher than maximum available number you can buy');
					}
					...
					$couponCount = CouponTable::getInstance()->createQuery('c')
									...
									//->groupBy('id')
									->count();
					if ($product['max_buy'] < ($count->s + $couponCount + $this->cart->fr_products[$product['id']]['qty'])) {
						$err = 1;
						$form->addError('Your friend already bought the maximum available number of this deal('.$product['name'].').');
					}
				}//end foreach fr products
				
				if ($redir == 1) {
					$this->redirect('/all-deals');
					die;
				}
...
//				}
				if ($spendFrom == 1) {
					if ($this->user->getUser()->get('wallet') < $totalCHK) {
						$err = 1;
						$form->addError('You do not have enought money in your wallet');
						$showDepositButton = true;
					}
				}
				if ($spendFrom == 2) {
					if ($this->user->getUser()->get('virtual_wallet') < $totalCHK) {
						$err = 1;
						$form->addError('You do not have enought money in your wmw wallet');
						//$showDepositButton = true;
					}
				}
				...
				//all is ok
				if ($err == 0) {
					foreach ($productsFORK AS $shopCartProduct) {
						$productObj = ProductTable::getInstance()->getProductById(array('product_id' => $shopCartProduct['product_id']))->fetchOne(array());
						$product = $productObj->toArray();
						
						$optionObj = ProductPriceTable::getInstance()->find($shopCartProduct['option_id']);
						//$quantity = $form->getValue('cart_qty_'.$product['id']);
						$quantity = $shopCartProduct['qty'];
						...
						try {
							...
							for ($k = 0; $k < $quantity; $k++) {
								...
								if ($spendFrom == 2) {
									$tran->transaction_type = $type['buy-wmw'];
								}
...
//								$tran->receiver_id = $product['user_id'];
								...
								if ($shopCartProduct['friend_id']) {
									$coupon->friend = 1;
									//$coupon->email = $form->getValue('email');
									$coupon->friend_id = $shopCartProduct['friend_id'];
...
//										$receiveTran->hint = 'Merchant commission received ('. $c .'), this will be sent in virtual_wallet';
                ...
                //$productObj->sold = $productObj->sold + $quantity;
							...
							if ($checkObj->get('sold') >= $checkObj->get('min_sold')) {
								...
								foreach ($tr as $t) {
									if ($t->transaction_type == $type['buy'] || $t->transaction_type == $type['buy-wmw']) {
										//lets get the coupon
...
//													He bought this product <a href="https://www.winmaweb.com/'.$po->username.'/'.$p->slug.'.html">'.$p->name.'</a><br /><br />
...
//											}
					...
					$this->redirect('/my-account/my-bought-deals');
				}
			}
		}
...
//						error_log($vv);
						...
						if ($vv > 0) {
							try {
                                                            ...
                                                            $pf = $pf['config_value'];
                                                            $fee = round($vv*$pf/100, 2);
                                                            $vv = $vv-$fee;
    //							$user = $this->user->getUser();
                                                            $usr = Doctrine_Core::getTable('User')->find($values['item_number']);
                                                            //Doctrine_Core::getTable('Transaction')->findOneBy('paypal_txn_id', $values['txn_id']);
    ...
    //							//the money from
                                                            ...
                                                            $sTran->save();
                                                            $sTran2 = new Transaction();
                                                            $sTran2->parent_id = $sTran->id;
                                                            $sTran2->transaction_type = $type['deposit-paypal-fee'];
    //							//the money from
                                                            $sTran2->sender_id = $values['item_number'];
                                                            $sTran2->receiver_id = $values['item_number'];

                                                            $sTran2->full_payment = $fee;
                                                            $sTran2->save();
    //							$success = true;
                                                            $conn->commit();
                                                        } catch (Exception $e) {
